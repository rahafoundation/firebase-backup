...
async function migrateMemberUsernamesDollarToDot(pathToFbKey: string, projectId: string): Promise<void> {
  const firebaseKey = require(pathToFbKey);
  if (firebaseKey.project_id !== projectId) {
    throw Error(`Gave projectId ${projectId} but path to firebase key credentials was for ${firebaseKey.project_id}`);
  }
  ...
  const GENESIS_UIDS = new Set(['mPTv77Bs8iVdPXUzF7bYv2zZ0qI2', 'sj7oal4vnLUALUhE5tb6xazgmuI3', 'dgSRh23dMcOXQeoA0vUyBE4cVAD2', 'Cj4TcLjooRe2D4WfIZyPAzdlUT52']);
  const numDocs = await db.runTransaction(async tx => {
    ...
    operations.docs.forEach(d => {
      ...
      if (!creatorMid || !toMid) {
        if (creatorUid === null && GENESIS_UIDS.has(toUid) && d.get('op_code') === 'TRUST') {
          // okay
        } else if (GENESIS_UIDS.has(creatorUid) && toUid === null && d.get('op_code') === 'REQUEST_INVITE') {
          // okay
        } else {
